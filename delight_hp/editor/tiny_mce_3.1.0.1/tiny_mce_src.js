...
tinymce.create('tinymce.util.Dispatcher', {
	...
	dispatch : function() {
		...
		for (i = 0; i<li.length; i++) {
			c = li[i];
			s = c.cb ? c.cb.apply(c.scope, a) : false;
...
(function() {
	...
	tinymce.create('tinymce.Editor', {
		...
		setParam : function(name, value, strip_whitespace, split_chr) {
			...
			// Fix bool values
			if (tr(value).toLowerCase() == "true" || tr(value).toLowerCase() == "false") {
				value = (tr(value).toLowerCase() == "true");
			}
		...
		getContent : function(o) {
			...
			if (!o.no_events)
				t.onBeforeGetContent.dispatch(t, o, true);
