...
TinyMCE_Engine.prototype = {
	...
	setParam : function(name, value, strip_whitespace, split_chr) {
		value = (typeof(value) != 'undefined') ? value : (typeof(this.settings[name]) != "undefined") ? this.settings[name] : '';
...
TinyMCE_Engine.prototype.cleanupHTMLCode = function(s) {
	...
	s = s.replace(new RegExp('<(h[1-6]|p|span|div|address|pre|form|table|li|ol|ul|td|b|font|em|strong|i|strike|u|span|a|ul|ol|li|blockquote)([a-z]*)([^\\\\|>]*)\\/>', 'gi'), '<$1$2$3></$1$2>');
...
TinyMCE_Engine.prototype.convertSpansToFonts = function(doc) {
	...
	for (var i=0; i<s.length; i++) {
		...
		// ADDED LUKAS: Remove empty Font-Tags
		if (s[i].attributes.length < 1) {
			s[i].parentNode.replaceChild(document.createTextNode(s[i].innerHTML), s[i]);
		}
	}
};
...
TinyMCE_Engine.prototype.convertFontsToSpans = function(doc) {
	...
	for (var i=0; i<s.length; i++) {
		...
		// ADDED LUKAS: Remove empty Span-Tags
		if (s[i].attributes.length < 1) {
			s[i].parentNode.replaceChild(document.createTextNode(s[i].innerHTML), s[i]);
		}
	}
...
TinyMCE_Engine.prototype._cleanupHTML = function(inst, doc, config, elm, visual, on_save, on_submit) {
	...
	if (doc.body.getElementsByTagName('p').length <= 0) {
		doc.body.innerHTML = doc.body.innerHTML + '<p>&nbsp;</p>';
	}
...
TinyMCE_Engine.prototype.getAbsPosition = function(n) {
	...
	// LUKAS: added and commented out - insert to take calc the abspos with scrolloffset
	/*if (document.getElementById('scrollcont')) {
		n = document.getElementById('scrollcont');
		if (typeof n.scrollTop == 'number') p.absTop -= n.scrollTop;
		else if (typeof n.pageYOffset == 'number') p.absTop -= n.scrollTop;
		if (typeof n.scrollLeft == 'number') p.absLeft -= n.scrollLeft;
		else if (typeof n.pageXOffset == 'number') p.absLeft -= n.scrollLeft;
	}*/
