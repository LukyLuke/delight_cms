	...
	switch($outputType) {
		case "xml":
			...
			if (count($result) == 0)
				...
				$body .= '<res id="' . $id . '" cmd="'. $cmd .'">'. urlencode(utf8_encode(implode(" ", $result))) .'</res>';
		...
		case "xmlerror";
			...
			$body  = '<?xml version="1.0" encoding="utf-8" ?>';
			$body .= "\n";
			$body .= '<res id="' . $id . '" cmd="'. $cmd .'" error="true" msg="'. utf8_encode(implode(" ", $tinyspell->errorMsg)) .'" />';
			echo $body;
		break;
