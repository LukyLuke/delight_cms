...
// delight BEGIN
function imsShowDelightImageList() {
	if (document.getElementById('delightimagesections') && document.getElementById('delightimagelist')) {
		var formObj = document.forms[0];
		var selected = formObj.src.value;
		dWebRequest.Init(true);
		dWebRequest.sendSectionRequest('delightimagesections', 'delightimagelist', selected, 'img', "setSection('[CONT]')");
	}
}

function setSection(value) {
	document.getElementById('delightimagesections').innerHTML = value;
//	imsDelightChangeSection({id : 'ims0'});
}
...
function imsDelightExpandSection(obj) {
	if (obj.nextSibling.style.display == 'none') {
		...
		if ( (obj.childNodes.length > 0) && (obj.childNodes.item(0).nodeName == 'IMG') && (obj.childNodes.item(0).alt == '+') ) {
			obj.childNodes.item(0).src = 'images/section_minus.gif';
		}
		obj.nextSibling.style.display = 'block';
	} else {
		// change the Image if there is a plus
		if ( (obj.childNodes.length > 0) && (obj.childNodes.item(0).nodeName == 'IMG') && (obj.childNodes.item(0).alt == '+') ) {
			obj.childNodes.item(0).src = 'images/section_plus.gif';
		}
...
function imsDelightChangeSection(obj) {
	if (document.getElementById('delightimagesections') && document.getElementById('delightimagelist')) {
		imsDelightSelectSection(obj.id);
		var formObj = document.forms[0];
		var selected = formObj.src.value;
		dWebRequest.Init(true);
		dWebRequest.sendContentRequest('delightimagelist', selected, parseInt(obj.id.substring(3)), 'imgshot', '', 'noadmin')
		//dWebRequest.sendContentRequest('delightimagelist', selected, parseInt(obj.id.substring(3)), 'img');
	}
}
function imsDelightSelectimage(obj) {
	if (obj.nextSibling && (obj.nextSibling.nodeName == 'INPUT')) {
		var formObj = document.forms[0];
		formObj.src.value = obj.nextSibling.value;
		showPreviewImage(formObj.src.value);
		mcTabs.displayTab('general_tab','general_panel');
	}
}
function imsDelightSelectSection(id) {
