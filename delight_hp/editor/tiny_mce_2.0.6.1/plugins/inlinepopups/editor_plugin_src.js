...
TinyMCE_Engine.prototype.openWindow = function(template, args) {
	...
	if (tinyMCE.getParam("popup_screen_center")) {
		var pos = {absLeft : 0, absTop : 0};
		if (self.innerHeight) { // all except Explorer
			pos.absLeft = self.innerWidth;
			pos.absTop = self.innerHeight;
		} else if (document.documentElement && document.documentElement.clientHeight) { // Explorer 6 Strict Mode
			pos.absLeft = document.documentElement.clientWidth;
			pos.absTop = document.documentElement.clientHeight;
		} else if (document.body) { // other Explorers
			pos.absLeft = document.body.clientWidth;
			pos.absTop = document.body.clientHeight;
		}
		pos.absLeft = Math.round((pos.absLeft / 2) - (width / 2));
		pos.absTop = Math.round((pos.absTop / 2) - (height / 2));
	} else {
