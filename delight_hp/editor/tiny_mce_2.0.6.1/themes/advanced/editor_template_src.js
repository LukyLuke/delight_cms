		...
		switch (button_name) {
			...
			case "fontsizeselect":
				return '<select id="{$editor_id}_fontSizeSelect" name="{$editor_id}_fontSizeSelect" onfocus="tinyMCE.addSelectAccessibility(event, this, window);" onchange="tinyMCE.execInstanceCommand(\'{$editor_id}\',\'FontSize\',false,this.options[this.selectedIndex].value);" class="mceSelectList">'+
						...
						'<option value="3">3 (10 pt)</option>'+
						'<option value="4">4 (11 pt)</option>'+
						'<option value="5">5 (12 pt)</option>'+
						'<option value="6">6 (14 pt)</option>'+
						'<option value="7">7 (16 pt)</option>'+
						'<option value="8">7 (18 pt)</option>'+
						'</select>';
		...
		if (selectElm) {
			if (!tinyMCE.isSafari && !tinyMCE.isOpera) {
				...
				if (elm) {
					...
					if (size == '') {
						var sizes = new Array('', '8pt', '9pt', '10pt', '11pt', '12pt', '14pt', '16pt', '18pt');
