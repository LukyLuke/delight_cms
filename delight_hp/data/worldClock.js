...
WorldClock.prototype = {
	startUpdate : function() {
		var t = this;
		if (!this.interval) {
			window.setInterval(function(e) {t.update();}, 1000);
		}
	},
	
	register : function(e, o, f) {
		if (!e) {
			return;
		}
		if (typeof(f) != 'string') {
	...
	update : function() {
		var i,obj,time,h,m,s;
		if (this.elements.length > 0) {
			for (i = 0; i < this.elements.length; i++) {
				obj = this.elements[i];
				time = new Date(this.getGMT() + (obj.offset * 3600000));
				obj.element.innerHTML = time.format(obj.format);
			}
		}
	},
...
if (typeof (Date.prototype.format) != 'function') {
	...
	Date.replaceChars = {
		...
		S : function() {
			return (this.getDate() % 10 == 1 && this.getDate() != 11 ? 'st'
					: (this.getDate() % 10 == 2 && this.getDate() != 12 ? 'nd'
							: (this.getDate() % 10 == 3 && this.getDate() != 13 ? 'rd'
									: 'th')));
		},
		w : function() {
			return this.getDay();
		...
		// Year
		L : function() {
			return (((this.getFullYear() % 4 == 0) && (this.getFullYear() % 100 != 0)) || (this
